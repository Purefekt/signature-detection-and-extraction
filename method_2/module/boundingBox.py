...
class BoundingBox:
    ...
    def bounding_boxes(self, mask, regions) -> dict:
        results = {}
        for key, region in regions.items():
            # crop region
        ...
        x = results[0]['cropped_region'][0]
        y = results[0]['cropped_region'][1]
        w = results[0]['cropped_region'][2]
        h = results[0]['cropped_region'][3]
    ...
    def run(self, np_image):
        ...
        # get x, y, w, h for bbox info
        return self.bounding_boxes(np_image, regions)